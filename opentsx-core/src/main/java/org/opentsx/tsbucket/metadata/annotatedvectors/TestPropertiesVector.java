package org.opentsx.tsbucket.metadata.annotatedvectors;
 ...
 * the units of measurement is already part of the org.opentsx.tsbucket.metadata: [s since t0, km/h]
 ...
 * information with each other. The "Internet of things" already uses this approach. All static org.opentsx.tsbucket.metadata about a device
 ...
 * Not the full set of org.opentsx.tsbucket.metadata is required in each data set, one URI us all what is needed a connecting link
 * to the org.opentsx.tsbucket.metadata layer.
 * 
 * Such explicit org.opentsx.tsbucket.metadata should not be used very often during data processing. To find out, what the brand and type of
 * a measurement device was it is good to use the external layer, but for correlation analysis such explicit org.opentsx.tsbucket.metadata
 ...
 * One has to be careful and should not be confused by explicit and implicit org.opentsx.tsbucket.metadata. Both types can be converted into
 * each other with data aggregation and ectraction procedures which finally form a well defined data set, for which relevant
 * connections to explicit datasets (in this case org.opentsx.tsbucket.metadata) are known. All this information is part of the DOAP file, which
 * allows automatic lookups of background information to datasets.
 * 
 * In many cases, the record org.opentsx.tsbucket.metadata is not required in each step of an analysis algorithm. E.g. the k-means classification
 ...
 * class to that id and this class can become part of the org.opentsx.tsbucket.metadata about the object and therefore it should be stored in the
 ...
 * what implementation and what parameters have been used to produce this results are part of the analysis step org.opentsx.tsbucket.metadata.
 * 
